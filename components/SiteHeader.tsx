...
export default function SiteHeader() {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    supabase.auth.getUser().then(({ data }) => { if (mounted) { setUserEmail(data.user?.email ?? null); setReady(true); } });
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => { if (mounted) { setUserEmail(session?.user?.email ?? null); setReady(true); } });
  ...
  return (
    <header className="site-header">
      <div className="container header-inner">
        <Link className="brand" href="/" prefetch aria-label={text.home}><span className="brand-mark">TT</span><span>TAHITIAN <span className="gradient">TRADER</span></span></Link>
        <nav className="nav" aria-label={text.nav}>
          ...
          {ready && accountLink}{ready && accountAction}{ready && <LanguageSwitcher />}{ready && primaryCta}
        </nav>
        <details className="mobile-nav">
          ...
          <div className="mobile-menu">
            ...
            {ready && accountLink}{ready && accountAction}{ready && <LanguageSwitcher />}{ready && mobilePrimaryCta}
          </div>
        </details>
      </div>
    </header>
