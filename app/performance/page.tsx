...
export default async function Performance() {
  ...
  const Report = ({ rows, empty }: { rows:any[] | null, empty:string }) => rows?.length ? <div className="grid" style={{marginTop:20}}>{rows.map((r:any)=><article className="card" style={{padding:26}} key={r.id}><span className="pill">{r.products?.platform?.toUpperCase()}</span><h3 style={{fontSize:22,margin:'14px 0 7px',overflowWrap:'anywhere'}}>{r.products?.name}</h3><p className="muted">{r.title}</p><pre style={{overflowX:'auto',padding:15,background:'var(--surface-2)',border:'1px solid var(--line)',borderRadius:12,fontSize:12}}>{JSON.stringify(r.metrics,null,2)}</pre></article>)}</div> : <div className="card" style={{padding:28,marginTop:20}}><b>{empty}</b><p className="muted">Verified data will be added when the reports are published.</p></div>;

  return <><SiteHeader /><main className="container section">
    <span className="eyebrow">Performance</span>
    <h1 style={{fontSize:'clamp(46px,6.5vw,72px)',lineHeight:1,letterSpacing:'-.06em',margin:'18px 0 14px',overflowWrap:'anywhere'}}>Evidence before claims.</h1>
    ...
    <div className="grid stats-grid" style={{gridTemplateColumns:'repeat(4,minmax(0,1fr))',marginTop:34}}>{['Net profit','Win rate','Max drawdown','Profit factor'].map(x=><div className="card stat" key={x}><span>{x}</span><strong>—</strong><small className="muted">Awaiting verified data</small></div>)}</div>
